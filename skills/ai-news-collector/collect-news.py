...
def write_news(items):
    ...
    for item in items:
        conn.execute(
            "INSERT OR REPLACE INTO news (id,title,title_cn,source,source_url,published_at,summary,summary_cn,tags,featured) VALUES (?,?,?,?,?,?,?,?,?,?)",
            [
                item.get('id', ''),
                item.get('title', ''),
                item.get('titleCn', ''),
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == '--items':
        ...
        if data.strip():
            ...
            print('usage: python3 collect-news.py --items \'[{"title":"..."}]\'')
            sys.exit(1)
